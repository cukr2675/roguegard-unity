...
{
    [AddComponentMenu("Ochalike Sprites/Ochalike Sprite Renderer")]
    public class OchalikeSpriteRenderer : MonoBehaviour, IOchalikeSpriteRenderController
    {
        /// <summary>
        ...
        private const int sortingGroupMemberCount = int.MaxValue; // 30;
        ...
        {
            parentPool.PoolOchalikeSpriteRenderer(this);
        }
        ...
        private struct Bone
        {
            ...
            {
#if UNITY_EDITOR
                if (spriteRenderer.sprite != null) { Debug.LogWarning($"スプライトの上書きが発生しました。 ({spriteRenderer.gameObject.name} -> {name})"); }
#endif
